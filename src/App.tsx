...
Return ONLY a valid JSON string (no markdown formatting, no \`\`\`json) with exactly this structure:
{
  ...
  const mergeProducts = (dProds: Product[], lProds: Product[]): Product[] => {
    ...
    if (Array.isArray(dProds)) dProds.forEach(addOrMerge);
    if (Array.isArray(lProds)) lProds.forEach(addOrMerge);
  ...
  const mergeLogs = (dLogs: PriceLog[], lLogs: PriceLog[]): PriceLog[] => {
    ...
    const addOrMerge = (log: PriceLog) => {
      if (!log || typeof log !== 'object' || !log.productId) return;
    ...
    if (Array.isArray(dLogs)) dLogs.forEach(addOrMerge);
    if (Array.isArray(lLogs)) lLogs.forEach(addOrMerge);
    return Array.from(map.values());
  };
  ...
  useEffect(() => {
    const initData = async () => {
      ...
      try {
        ...
        if (Array.isArray(prodData) && prodData.length > 0) dbProds = prodData;
        if (Array.isArray(logData) && logData.length > 0) dbLogs = logData;
      } catch (err) {
      ...
      try {
        ...
        console.error("Data merge error, fallback to initial:", err);
        setProducts(INITIAL_PRODUCTS);
        setPriceLogs(generateHistoricalLogs());
      } finally {
        setIsLoading(false);
      }
    };
